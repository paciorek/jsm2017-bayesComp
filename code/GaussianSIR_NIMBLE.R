...
dev.print(dev=pdf,file="Gaussian_means_NIMBLE.pdf")
...
lines(mm.SIR[-1]+2*sqrt(CC2.SIR[-1]), col="blue", lwd=1, lty=3)
legend(120,3,c("Exact","SIR"), col=c("red","blue"), lty=c(2,3), lwd=2, bty="n")
dev.print(dev=pdf,file="Gaussian_confint_NIMBLE.pdf")


## ESS as a function of time
quartz()
plot(ESS[-1], type="l", ylab="ESS", xlab="t",ylim=c(0,BB))
dev.print(dev=pdf,file="Gaussian_ESSintime_NIMBLE.pdf")
...
dev.print(dev=pdf,file="Gaussian_ESSvslogVar_NIMBLE.pdf")
